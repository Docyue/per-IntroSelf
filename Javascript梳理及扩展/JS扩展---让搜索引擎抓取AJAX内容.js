...
　　http://example.com#3
　　
但是，搜索引擎只抓取example.com，不会理会#号，因此也就无法索引内容。
为了解决这个问题，Google提出了"#号+感叹号"的结构。
　　http://example.com#!1

--------------------------------------------------------------------------------　
当Google发现上面这样的URL，就自动抓取另一个网址：
　　http://example.com/?_escaped_fragment_=1
只要你把AJAX内容放在这个网址，Google就会收录。但是问题是，"#号+感叹号"非常难看且烦琐。Twitter曾经采用这种结构，它把
...
--------------------------------------------------------------------------------
最后，设置服务器端，因为不使用井号结构，每个URL都是一个不同的请求。所以，要求服务器端对所有这些请求，都返回如下结构的网页，防止出现404错误。
